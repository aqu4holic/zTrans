...
def process_and_expand_data(data):
    ...
    for index in tqdm(range(len(data)), desc = 'parsing data'):
        try:
            ...
            if len(methods_start) == 0: continue
            if len(methods_end) == 0: continue
            total_methods_before.append(len(methods_start))
            total_methods_after.append(len(methods_end))
...
def main():
    ...
    expanded_data.to_parquet('/drive2/phatnt/zTrans/data/data_method_treesitter1_abstract_class_declaration.parquet')
